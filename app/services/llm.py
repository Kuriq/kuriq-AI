...
중요 규칙:
1. **반드시 제공된 후보 강좌 목록의 [인덱스] 만 사용하세요.**
2. **목록에 없는 인덱스를 절대 만들지 마세요.**
3. **courseId 를 직접 입력하지 말고, 인덱스 번호만 사용하세요.**
4. 난이도 순서로 배열하세요 (입문 → 초급 → 중급 → 심화).
5. 주당 총 학습 시간이 weeklyHours 를 크게 벗어나지 않도록 조정하세요.
...
def generate_roadmap(
    ...
    if total_valid_courses == 0:
        ...
        logger.error(f"[LLM] LLM 이 반환한 courseIndex: {[c.get('courseIndex') for w in data.get('weeks', []) for c in w.get('courses', [])]}")
