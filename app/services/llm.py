...
출력 형식:
{{
  "title": "간단한 로드맵 제목 (예: AI·데이터 입문 로드맵)",
  "goal": "목표 요약 (한 문장)",
...
def _build_fallback_title(intent: ExtractedIntent, goal: str) -> str:
    ...
    subject = "맞춤 학습"
    if any(keyword in source for keyword in ["ai", "인공지능", "데이터", "머신러닝", "딥러닝"]):
        subject = "AI·데이터"
    elif "영어" in source:
        subject = "영어"
    elif "일본어" in source:
        ...
        subject = "파이썬"
    elif any(keyword in source for keyword in ["프로그래밍", "코딩", "개발"]):
        subject = "프로그래밍"
    elif any(keyword in source for keyword in ["디자인", "ui", "ux"]):
        subject = "디자인"
    elif "마케팅" in source:
        subject = "마케팅"
    elif any(keyword in source for keyword in ["경영", "비즈니스"]):
        subject = "경영"
